...
当前版本为**无状态服务**，不依赖数据库。如需持久化提取结果以支持历史查询和趋势分析，建议添加：

### 推荐数据库

- **开发/测试**：H2（Spring Boot 内嵌，零配置）
- **生产**：PostgreSQL 14+ 或 MySQL 8.0+
...
CREATE TABLE conversations (
    ...
    channel         VARCHAR(8)   NOT NULL,   -- 在线/电话
    agent_name      VARCHAR(32)  NOT NULL,
    raw_json        TEXT         NOT NULL,   -- 原始对话 JSON
...
CREATE TABLE extraction_results (
    ...
    user_primary_intent VARCHAR(200),
    is_resolved         BOOLEAN,
    resolution_type     VARCHAR(32),
    user_initial_sentiment VARCHAR(16),
    user_final_sentiment   VARCHAR(16),
    sentiment_trend     VARCHAR(16),
    has_multiple_intents BOOLEAN DEFAULT FALSE,
    has_escalation      BOOLEAN DEFAULT FALSE,
    is_incomplete       BOOLEAN DEFAULT FALSE,
    agent_solution_quality VARCHAR(16),
    needs_follow_up     BOOLEAN DEFAULT FALSE,
    result_json         TEXT,              -- 完整提取结果 JSON
    extracted_at        TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    extract_mode        VARCHAR(8)         -- llm / mock
);

CREATE INDEX idx_extraction_intent ON extraction_results(intent_category);
CREATE INDEX idx_extraction_resolved ON extraction_results(is_resolved);
CREATE INDEX idx_extraction_followup ON extraction_results(needs_follow_up);
CREATE INDEX idx_extraction_agent ON extraction_results(conversation_id);
```

### 你需要做什么

1. 在 `build.gradle` 添加：`implementation 'org.springframework.boot:spring-boot-starter-data-jpa'` 和数据库驱动
2. 创建 JPA Entity 类映射上述表
3. 创建 Repository 接口
4. 修改 `ExtractionRunner` 写入数据库而非仅 JSON 文件
5. 添加查询 API：按时间/意图/情绪/客服等维度统计

---

## 9. 与 extract_example.md 的差异

示例只有一个简单 schema，本项目在此基础上做了以下扩展：

- **意图分类**：从无分类 → 11 类细分
- **情绪分析**：从单点情绪 → 始末 + 趋势
- **边界标记**：新增 4 个标记（多诉求/转人工/话题切换/未完成）
- **实体抽取**：新增订单号、商品名识别
- **客服评估**：新增客服解决质量维度
- **可操作标签**：新增 needs_follow_up、key_tags
